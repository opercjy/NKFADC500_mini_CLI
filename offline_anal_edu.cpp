...
void offline_anal_edu(const char* filename = "data/run_101_prod.root", int targetCh = 0, double ampCut = 50.0, double dtCut_us = 10.0) {
    ...
    TTreeReaderValue<ULong64_t> triggerTime(reader, "TriggerTime"); 
    ...
    while (reader.Next()) {
        ...
        if (!isFirst) {
            double dt_us = (currTime - prevTime) / 1000.0; 
            hDeltaT->Fill(dt_us);
        }
        prevTime = currTime;
        isFirst = false;
    ...
    tree->Draw(Form("Amplitude[%d]:(Charge[%d] * %f)>>h2D(200,0,1000, 200,-100,4200)", targetCh, targetCh, adc_to_pc), "", "colz");
