...
void BinaryDaqManager::ConsumerWorker(const std::string& outFileName, int maxEvents) {
    ...
    while (fIsRunning || fDataQueue.Size() > 0) {
        ...
        auto current_time = std::chrono::steady_clock::now();
        double elapsed_sec = std::chrono::duration<double>(current_time - ui_timer).count();
        
        if (elapsed_sec >= 0.5) {
            double speed_mbps = (((total_written_bytes - last_print_bytes) / 1048576.0) / elapsed_sec);
            double evt_rate = (current_events - last_print_events) / elapsed_sec;
