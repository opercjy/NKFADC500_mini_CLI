...
Notice Korea의 FADC500 Mini (500MS/s, 12-bit, 4-Channel) 보드를 제어하고, 초고속으로 바이너리 데이터를 수집하여 ROOT 프레임워크 기반으로 분석하기 위한 고성능 하이브리드 C++ / Python 통합 DAQ 아키텍처입니다.

극단적인 고속 트리거(High Noise & Trigger Rate) 환경에서도 USB 버퍼가 뻗지 않도록 C++ 백엔드의 무결성 방어 로직(Zero-Deadlock, 100% Drain)이 적용되어 있으며, 이를 제어할 수 있는 PyQt5 기반의 마스터 컨트롤 GUI 패널이 통합되어 있습니다. 특히, USB 3.0 초고속 데이터 획득 환경에서 발생하는 화면 렌더링 병목 현상을 완벽하게 극복한 비동기 온라인 모니터링 시스템을 자랑합니다.

## 1. 시스템 아키텍처 개요 (System Architecture)

본 시스템은 수집(Online), 감시(Monitoring), 분석(Offline)의 병목을 완벽히 분리한 하이브리드 아키텍처를 가집니다. 최근 패치를 통해 백엔드 프로세스의 안정적인 종료(Graceful Shutdown 및 SIGKILL Fallback)와 상태 동기화 로직이 대폭 강화되었습니다.
...
* **[Master GUI] PyQt5 Control Panel (`fadc500_gui`) : UI/UX 전면 개편 및 최적화 완료 (Stable)**
...
본 프로젝트로 완성된 통합 GUI 패널은 복잡한 설정 파일과 터미널 명령어 입력의 고통을 덜어주고, 직관적인 대시보드를 통해 실험의 가시성을 극대화합니다. 최신 업데이트를 통해 수동 제어(Manual) 패널과 임계값 스캔(Auto Scan) 패널이 스플릿 화면(Side-by-Side)으로 재설계되어 운용 효율이 향상되었습니다.
...
* **하이브리드 시스템 콘솔:** C++ 백엔드에서 출력되는 순수 텍스트, ANSI 색상 코드, 그리고 Python 내부의 HTML 태그를 완벽하게 분리하여 렌더링하는 지능형 파서를 탑재하여 시스템 로그의 가독성을 극대화. 명령어 인자 등 불필요한 로그는 사용자 화면에서 숨김(Hidden) 처리하여 깔끔한 모니터링 환경 제공.
* **비동기 모니터 제어:** 마스터 콘솔에서 Live Monitor ON/OFF 제어 및 Clear Monitor 명령을 통해 백그라운드로 구동 중인 C++ 라이브 뷰어의 상태를 실시간 제어. 런(Run) 변경 및 스캔 단계 변경 시 모니터 뷰어 자동 갱신.
...
│   ├── core/               # IPC(프로세스간 통신), 서브프로세스 정밀 파싱 및 감시 매니저
│   ├── windows/            # 하이브리드 파서가 탑재된 메인 윈도우 레이아웃 오케스트레이터
...
# ---------------------------------------------------
# 1) 프론트엔드 수집 가동
../bin/frontend_500_mini -f ../config/settings.cfg -o ../data/run_0001.dat -n 10000
...
* [x] **Phase 5.5:** 고대역폭 수신/렌더링 분리 아키텍처 및 통신망(IPC) 정밀 파싱 업그레이드
* [x] **Phase 6:** GUI 대시보드 UI/UX 전면 개편 (하이브리드 콘솔 출력 및 분할 패널 적용)
* [x] **Phase 7:** 교육 및 연구 목적의 물리량(pC) 기반 고급 오프라인 분석 파이프라인 스크립트 작성
* [ ] **Phase 8 (진행 중):** CAEN HV(고전압) 전원장치 소프트웨어 연동 제어 및 채널 마스킹(소프트웨어 리패킹 엔진)을 통한 데이터 획득(DAQ) 극한 성능 최적화

## 6. 교육 및 연구용 고급 오프라인 분석 파이프라인 (Advanced Educational Analysis)

본 시스템은 단순한 데이터 수집을 넘어, 입자물리학 데이터를 분석하고 검출기의 특성을 파악하는 정석적인 방법론을 제공합니다. 데이터는 교육 및 분석에 최적화된 플랫 트리(Flat Tree) 구조로 저장되며, NKFADC500 Mini 장비의 하드웨어 스펙(12-bit 해상도, 5Vpp 동적 범위, 500MS/s, 50Ohm 임피던스)을 반영하여 **ADC 카운트를 순수 물리량(pC)으로 정밀 환산(0.048828125 pC/ADC)**하는 로직이 적용된 분석 스크립트들을 제공합니다.

### 제공되는 핵심 분석 스크립트

**1. 기초 분석 및 노이즈 컷(Cut) 필터링 (`offline_anal_edu.cpp`)**
...
### 분석 스크립트 실행 방법
분석 환경(ROOT)이 로드된 상태에서 터미널에 아래 명령어를 입력하십시오. (모든 스크립트의 UI 및 출력은 한글 깨짐을 방지하기 위해 순수 영어로 작성되었습니다.)

```bash
# 기본 분석 대시보드 실행 (인자: 파일명, 채널, 진폭 컷, Delta T 컷)
root -l 'offline_anal_edu.cpp("data/run_0001_prod.root", 0, 50.0, 10.0)'

# 지능형 SPE 캘리브레이션 및 Absolute Gain 도출 (인자: 파일명, 채널)
root -l 'offline_spe.cpp("data/run_0001_prod.root", 0)'

# 컴프턴 엣지 정밀 피팅 (인자: 파일명, 채널, X축 최대 탐색 범위)
